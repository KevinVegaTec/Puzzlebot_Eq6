...
class LineDetector:
    def __init__(self):
        ...
        # self.steer_slowdown_amount = rospy.get_param("steering_slowdown")
        self.image_sub = rospy.Subscriber("/video_source/raw", Image, self.image_callback_raw,queue_size=1)
        self.image_pub = rospy.Publisher("processed_image", Image,queue_size=1)
        self.speed_pub = rospy.Publisher('/motor_driver/speed', Float32,queue_size=1)
        self.dir_pub = rospy.Publisher('/motor_driver/direction', Float32,queue_size=1)
    ...
    def process_image(self, cv_image):
        ...
        #cv Change to grayscale and blur a bit
        cv_image = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY)
        cv_image = cv2.GaussianBlur(cv_image, (7, 7), 0)

        # Threshold the image
        (T, threshold_image) = cv2.threshold(cv_image, 100, 255, cv2.THRESH_BINARY)
        ...
        image_message = self.bridge.cv2_to_imgmsg(threshold_image, "rgb8")
        self.image_pub.publish(image_message)

        # Output the control speed / direction
        # self.dir_target = self.line_offset * self.steer_multiplier
        # self.dir_slowdown = 1.0 - (abs(self.dir_target) * self.steer_slowdown_amount)
        ...
        speed.angular.x = 0
        speed.angular.y = 0
        speed.angular.z = self.line_offset
